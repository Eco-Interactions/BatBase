/**
 * Builds and returns the subForm according to the passed params. Disables the
 * select elem 'parent' of the sub-form.
 * (container)DIV>[(header)P, (fields)DIV, (buttons)DIV]
 */
import { _elems, _cmbx, _state } from '../../forms-main.js';
import { _modal, _u } from '../../../db-main.js';

let entity, fClasses, fLvl, selId;

export default function getSubForm(formLvl, classes, fVals, sId) {
    fClasses = classes,
    fLvl = formLvl,
    selId = sId,
    entity = _state('getFormProp', [formLvl, 'entity']);
    return _elems('buildFormRows', [entity, fVals, fLvl])
        .then(buildFormContainer)

}
function buildFormContainer(rows) {
    const subFormContainer = buildSubFormCntnr();
    const bttns = _elems('getFormFooter', [entity, fLvl, 'create']);
    $(subFormContainer).append([getTutorialBttn(), buildFormHdr(), rows, bttns]);
    _state('setFormProp', [fLvl, 'pSelId', selId]);
    _cmbx('enableCombobox', [selId, false]);
    return subFormContainer;
}
function getTutorialBttn() {
    const cntnr = _u('buildElem', ['div', { class: 'flex-row'}]);
    $(cntnr).append(getFormTutorialButton).css({'justify-content': 'flex-end'});
    return cntnr;
}
function getFormTutorialButton() {
    const bttnTxt = _u('ucfirst', [entity]) + ' Form Tutorial';
    const attr = { class: 'ag-fresh', type: 'button', value: bttnTxt };
    const bttn = _u('buildElem', ['input', attr]);
    $(bttn).click(_modal.bind(null, 'showFormTutorial', ['top']));
    return bttn;
}
function buildSubFormCntnr() {
    const attr = {id: fLvl+'-form', class: fClasses };
    return _u('buildElem', ['div', attr]);
}
function buildFormHdr() {
    const attr = { text: 'New '+_u('ucfirst', [entity]), id: fLvl+'-hdr' };
    return _u('buildElem', ['p', attr]);
}