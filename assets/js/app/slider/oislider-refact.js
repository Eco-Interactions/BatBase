$(function() {	/* framePlayer object */	
	const frameSet = buildFrameSet();
	const frames = buildFrames();
	const confg = buildSliderConfig();
	ECO_INT_FMWK.framePlayer = initSlider();
	...
	function initSlider() {
		...
		function initPlayer() {
			...
			function getSliderCfg() {	/* Function Defs are in this scope so Pointers are passed to events from here */
				confg.events = {};
				confg.events.afterInitialize = initFramePlayer;	/* initFramePlayer is internal function of framePlayer */
				confg.events.afterSlide = autoNextFrame; /* autoNextFrame is internal function of framePlayer */
				return confg;
			}
			...
			function initFrames() {
				...
				var newFrames = [];
				$.each(frames, expandRawFrame);
				$.each(frames, createGhost);	 //	console.log('frames:%O', frames);
				$.each(frames, createNewFrame);
				...
				function completeFrame(frameIdx, newFrame) {
					var rFm = frames[frameIdx];
					var prvRawFm = frames[rFm.prevIdx];
				...
				function expandRawFrame(frameIdx, rFm) {
					...
					var showsGhostOfPrevBeforeDisplay = ('ghostGrp' in frames[prevFmIdx]) ? true : false;
					var allCss = getCss();
					rFm.idx = frameIdx;
