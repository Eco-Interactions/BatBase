...
function loadCountryAndSubLocs(cntrys, r, query) {
    const cntryId = getCountryId(cntrys, r, query);
    app.volatile.prnt = cntryId;
    if (app.map._container.id === 'map') { return addParentLocDataToMap(cntryId, 'noZoom', 'map'); }
    selectCountryInForm(cntryId, $('#form-map').data('type'));
    addParentLocDataToMap(cntryId, app.volatile.poly);
}
function selectCountryInForm(id, formType) {
    const cntry = formType === 'int' ? 'Country-Region' : 'Country';
    $(`#sel-${cntry}`)[0].selectize.addItem(id, 'silent');
}
...
function buildLocData(latLng, results, cntrys) {                                console.log('--buildLocData. latLng = %O results = %O', latLng, results);
    ...
    return {
        cntryId: getCountryId(cntrys, results.properties, query),
        lat: latLng.lat,
        lng: latLng.lng,
        name: results.name,
    };
...
function getCntryFromCode(cntrys, cntryCode) {
...
function ifNameInResult(tName, resultName, query) {
    return tName.indexOf(query) !== -1 || resultName.indexOf(tName) !== -1;
}
function ifTerritory(tName, rData) {
    if (!rData) { return; }
