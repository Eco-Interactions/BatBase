...
class Version20180927162956DisplayPoint extends AbstractMigration implements ContainerAwareInterface
{
    ...
    private function combineIndias($stay, $delete)
    {
        ...
        $stayLoc->setDisplayName($newName);
        $this->updateHabitats($stayLoc, $newName);
    }
    private function updateHabitats($loc, $name)
    {
        foreach ($loc->getChildLocs() as $childLoc) {
            if ($childLoc->getLocationType()->getDisplayName() === 'Habitat') {
                $parts = explode('-', $childLoc->getDisplayName());
                array_splice($parts, 0, 1, [$name]);
                $childLoc->setDisplayName(join($parts));
                $this->em->persist($childLoc);
            }
        }
    }
    private function deleteLocs($loc, $name)
    {
        foreach ($loc->getChildLocs() as $childLoc) {
        ...
        $loc->setDisplayName($name.'-removed');
        $this->em->persist($loc);
        $this->em->flush();
        $this->em->remove($loc);
        $this->em->flush();
    ...
    private function updateDisplayPoint()
    {
        $cntrys = $this->getDisplayPoints();

        foreach ($cntrys as $cntry => $point) {
            $this->addPointToGeoJson($cntry, $this->encodePoint($point));
        }
    }
    /**
     ...
     */
    private function encodePoint($point)
    {
    ...
    private function getDisplayPoints()
    {
        return [
            "Côte d'Ivoire" => [7.5400, -5.5471],
            "Bonaire, Sint Eustatius and Saba" => [12.1784, -68.2385],
            "Disputed Territory [includes the Paracel Islands and Spratly Islands]" => [8.6333308, 111.916663],
            "Svalbard and Jan Mayen" => [8.6333308, 111.916663],
            "Bouvet Island" => [-54.4208, 3.3464],
            "Guadeloupe" => [16.2650, -61.5510],
            "Martinique" => [14.6415, -61.0242],
            "Saint Bathélemy" => [17.9000, -62.8333],
