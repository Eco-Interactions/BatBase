$(function(){
	...
	self.init =  function() {
		...
		function initStepData(s) {
			...
			expandStepObjects(s.msgCtnr);
																		 console.log('stepObjs = %O', stepObjs);
		}
		...
		function startStep(s, stepIdx) {				//			console.log('startStep passed stepIdx %s', stepIdx);
			...
			(hasGhostClass(step) && addGhost(step)) || visibleFade();
		}
		...
		function visibleFade() {
			var step = stepObjs[s.stepIdx];
			var $sel = hasGhostClass(step) ? $(step.fadeGhostClass) : msgsToFade(step);
		...
		function hasGhostClass(step) {
			return 'ghostClass' in step;
		}
		...
		function getNextStepIdx(s, reverse) {
			s.ghostClass = 'ghostClass' in stepObjs[s.stepIdx] ? stepObjs[s.stepIdx].ghostClass : false;
		...
		function expandStepObjects(msgCtnr) {
			...
			var ghostHtml = getGhostHtml(msgCtnr);		// console.log('ghostHtml = %s', ghostHtml);
			var $allGhosts = $();
			var fadeGroupSelections = {};
			$.each(stepObjs, expandStep);

			console.log('fadeGroupSelections = %O', fadeGroupSelections);
			console.log('$allGhosts = %O', $allGhosts);
			
//			s.ghostClass = 'ghostClass' in stepObjs[0] ? stepObjs[0].ghostClass : false;

			function expandStep(stepIdx, step) {
				step.idx = stepIdx;
				step.nextIdx = (stepIdx + 1) < stepCnt ? (stepIdx + 1) : 0;
				step.prevIdx = (stepIdx > 0 ? stepIdx : stepCnt) - 1;
				step.ghost = getGhost(step);		console.log('st-%s expandStep set step.ghost = %O', step.idx, step.ghost);
				step.fades = getFades(step);
				


				// store fadedGhosts faded this step TBD

				function getFades(step) {			// console.log('st-%s getFades | step = %s', step.idx, JSON.stringify(step));
					return hasKey(step, 'fadeGroupNames') ? selectFades(step) : false;
				}

				function selectFades(step) {		 
					var fadeGroupNameAry = step.fadeGroupNames.split(',');			console.log('st-%s selectFades set fadeGroupNameAry = %O', step.idx, fadeGroupNameAry);
					return combineFades(fadeGroupNameAry) ;
					
					function combineFades(fadeGroupNameAry) {
						var $fadeSel = $();
						$.each(fadeGroupNameAry, addFade);						console.log('st-%s combineFades returning = %O', step.idx, $fadeSel);
						return $fadeSel;
				...
				function getGhost(step) {
					var prevStep = stepObjs[step.prevIdx];
					return hasKey(prevStep, 'ghostClass') ? buildGhost(prevStep) : false;
				}

				function buildGhost(prevStep) {				//	console.log('buildGhost running for step %s w/ ghostHtml = %s', step.idx, ghostHtml);
//					var elem = $(ghostHtml).addClass(prevStep.ghostClass);							//	console.log('elem = %O', elem);
					var msg = prevStep.msgTxt;			//	console.log('msg = %s', msg);
					var $newGhost = elem.html(msg);
					
//					var $newGhost = $(ghostHtml).html(stepObjs[step.prevIdx].msgTxt);
																	  console.log('st-%s buildGhost set newGhost = %s', step.idx, JSON.stringify($newGhost));
					addToFadeGroup($newGhost, prevStep.ghostClass);						// TODO - Rename ghostClass to ghostGroup throughout
																	//  console.log('st-%s buildGhost returning %s', step.idx, JSON.stringify($newGhost));
					$allGhosts = $allGhosts.add($newGhost);
					return $newGhost;
				}

				function addToFadeGroup($ghost, groupName) {		console.log('st-%s addToFadeGroup passed groupName = %s and $ghost = %O', step.idx, groupName, $ghost);
					hasKey(fadeGroupSelections, groupName) || addGroup(groupName);		console.log('...and adding it to %s', groupName);
					var $grpSelection = fadeGroupSelections[groupName];
					fadeGroupSelections[groupName] = $grpSelection.add($ghost);		console.log('st-%s addToFadeGroup added $ghost | fadeGroupSelections = ', groupName, fadeGroupSelections);
				}

				function addGroup(groupName) {
					fadeGroupSelections[groupName] = $();	console.log('addGroup added key %s w/ empty selection %O', groupName, fadeGroupSelections[groupName]);
				}
