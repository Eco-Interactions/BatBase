...
/*-------------- Form Builders -------------------------------------------------------------------*/
    ...
    function buildCitationText(fLvl) {
        ...
        function getTitlesAndEditors() { 
            const chap = formVals.chapterTitle ? 
                stripString(formVals.chapterTitle) : false;
    ...
    function updateSrcDetailPanel(entity) {
        ...
        function buildSourceData() {
            ...
            function addCitTitleData() {
                const citType =  getSrcType(cit, 'citation'); 
                ...
                function getCitTitle() {
                    if (!cit) { return false; }
                    const citTitle = cit.displayName.split('(citation)')[0];
                    return citTitle === pub.displayName ? false : citTitle;
                }
            } /* End addCitTitleData */
            function addAuthsAndEds() {
                const rcrdWithAuths = Object.keys(pubSrc.authors).length ? pubSrc : 
                    citSrc && Object.keys(citSrc.authors).length ? citSrc : false; 
                if (!rcrdWithAuths) { return; }
                data.Authors = getAuthorNames(rcrdWithAuths);
                if (Object.keys(pubSrc.editors).length) {
     ...
     * the selected type. 
     */
    function updateFieldLabelsForType(entity, typeId, elem, fLvl) { 
        const typeElemId = '#'+_util.ucfirst(entity)+'Type-sel'; 
...
/*------------------------------ Taxon ---------------------------------------*/
    ...
    function initAuthForm(value) {                                              //console.log("Adding new auth! val = %s", val);      
        handleNewAuthForm(value, 'Authors');
    }
    function initEdForm(value) {                                                //console.log("Adding new auth! val = %s", val);
        handleNewAuthForm(value, 'Editors');
    }
    /**
     ...
     */
    function handleNewAuthForm(value, authType) {
        const parentSelId = getParentSelId(); 
        const fLvl = getSubFormLvl('sub2');
        const singular = authType.slice(0, -1);
        const val = value === 'create' ? '' : value;
        ...
        handleSubmitBttns(fLvl);
        $('#FirstName_row input').focus();
        return { 'value': '', 'text': 'Creating '+singular+'...' };

        function getParentSelId() {
            const authCnt = $('#'+authType+'-sel-cntnr').data('cnt');
            return '#'+authType+'-sel'+authCnt;
        }
        function handleSubmitBttns(fLvl) {
            const prntLvl = getNextFormLevel('parent', fLvl);
            enableSubmitBttn('#'+fLvl+'-submit');
            disableSubmitBttn('#'+prntLvl+'-submit');
        }
    ...
    function toggleShowAllFields(entity, fLvl) {                                //console.log('--- Showing all Fields [%s] -------', this.checked);
        ...
        fillComplexFormFields(fLvl);
        if (entity === 'citation') { handleCitText(fLvl); }
    }
    /**
    ...
    function buildRow(field, fieldsObj, entity, fVals, fLvl) {                  //console.log("buildRow. field [%s], fLvl [%s], fVals = %O, fieldsObj = %O", field, fLvl, fVals, fieldsObj);
        ...
        function fillFieldIfValuePassed() {
            if (field in fVals) { 
                if (fieldsObj.fields[field] === "multiSelect") { return; }
